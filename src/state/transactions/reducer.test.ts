...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.KALYCHAIN]).toBeTruthy()
      expect(txs[ChainId.KALYCHAIN]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.KALYCHAIN]?.['0x0']
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.KALYCHAIN), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.KALYCHAIN] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.KALYCHAIN }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.KALYCHAIN), String(ChainId.RINKEBY)])
      expect(Object.keys(store.getState()[ChainId.KALYCHAIN] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.RINKEBY] ?? {})).toEqual(['0x1'])
    })
  })
})
