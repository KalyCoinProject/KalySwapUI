...
import { KLC, ChainId, Currency, Pair, Token, TokenAmount, Chain, KALYCHAIN_MAINNET } from '@kalycoinproject/sdk'
...
export function useGetWalletChainTokens() {
  ...
  const chainId = useChainId()
  let chain = useChain(chainId)
  // This functions is temporary for Kalyswap birthday
  const getKalyswapPairs = async () => {
    const query = qs.stringify(
      {
        id: account,
        protocol_id: 'klc_kalyswap'
      },
      {
        encodeValuesOnly: true
      }
  ...
  const getBalance = async () => {
    if (account && chainId) {
      ...
      const requestTokens: TokenDataUser[] = data.map((token: any) => {
        if (token?.id?.toLowerCase() === 'klc') {
          return new TokenDataUser(KLC[chainId], token?.price, token?.amount)
        }
      ...
      if (chainId === ChainId.KALYCHAIN) {
        const pairs = await getKalyswapPairs()
