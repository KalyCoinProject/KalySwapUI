...
import { TokenAmount, Token, ChainId, Percent, JSBI } from '@kalycoinproject/sdk'
...
describe('utils', () => {
  describe('#getEtherscanLink', () => {
    it('correct for tx', () => {
      expect(getEtherscanLink(ChainId.KALYCHAIN, 'abc', 'transaction')).toEqual('https://snowtrace.io/tx/abc')
    })
    it('correct for token', () => {
      expect(getEtherscanLink(ChainId.KALYCHAIN, 'abc', 'token')).toEqual('https://snowtrace.io/token/abc')
    })
    it('correct for address', () => {
      expect(getEtherscanLink(ChainId.KALYCHAIN, 'abc', 'address')).toEqual('https://snowtrace.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(getEtherscanLink(ChainId.TESTNET, 'abc', 'address')).toEqual('https://testnet.snowtrace.io/address/abc')
    })
    it('testnet', () => {
      expect(getEtherscanLink(ChainId.TESTNET, 'abc', 'address')).toEqual('https://testnet.snowtrace.io/address/abc')
    })
