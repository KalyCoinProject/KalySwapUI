...
import { BIG_INT_ZERO, KALYSWAP_API_BASE_URL } from '../../constants'
      ...
      Promise.all(
        stakingInfos
          ...
          .map(stakingInfo => {
            if (poolMap) {
              return fetch(
                `${KALYSWAP_API_BASE_URL}/kalyswap/apr2/${poolMap[stakingInfo.totalStakedAmount.token.address]}`
              )
                .then(res => res.json())
                .then(res => ({
              ...
              return fetch(`${KALYSWAP_API_BASE_URL}/kalyswap/apr/${stakingInfo.stakingRewardAddress}`)
                .then(res => res.json())
                .then(res => ({
                  swapFeeApr: Number(res.swapFeeApr),
                  stakingApr: Number(res.stakingApr),
      ...
      <TopSection gap="md">
        <DataCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>{t('earnPage.kalyswapLiquidityMining')}</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>{t('earnPage.depositKalyswapLiquidity')}</TYPE.white>
              </RowBetween>{' '}
          ...
          <DataCard>
            ...
            <CardSection>
              <AutoColumn gap="md">
                ...
                <RowBetween>
                  <TYPE.white fontSize={14}>{t('earnPage.kalyswapGovernanceProposalResult')}</TYPE.white>
                </RowBetween>
                {version !== '0' && (
                  <NavLink style={{ color: 'white', textDecoration: 'underline' }} to="/kswap/0">
                    <TYPE.white fontSize={14}>{t('earnPage.oldKswapPools')}</TYPE.white>
                  </NavLink>
                )}
              </AutoColumn>
            </CardSection>
