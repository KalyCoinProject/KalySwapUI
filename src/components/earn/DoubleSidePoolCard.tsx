...
export default function DoubleSidePoolCard({
  ...
  const token: Token =
    currency0 === KLC[chainId] || currency1 === KLC[chainId]
      ? currency0 === KLC[chainId]
        ? token1
        : token0
      : token0.equals(KSWAP[token0.chainId])
      ? token1
      : token0
      ...
      <TopSection isStaking={isStaking}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          {(isStaking || !stakingInfo.isPeriodFinished) && (
            <StyledInternalLink
              to={`/kswap/${currencyId(currency0, chainId)}/${currencyId(currency1, chainId)}/${version}`}
              style={{ width: '100%' }}
            >
          ...
          <Box>
            <BottomSection showBackground={true}>
              ...
              <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
                ...
                {`${stakingInfo.rewardRatePerWeek?.toSignificant(4, { groupSeparator: ',' })} KSWAP / week`}
              </TYPE.black>
            </BottomSection>
