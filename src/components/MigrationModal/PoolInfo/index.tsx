...
import { Text, Box, DoubleCurrencyLogo, Steps, Step } from '@kalycoinproject/components'
import { Pair, TokenAmount, JSBI } from '@kalycoinproject/sdk'
...
const PoolInfo = ({
  ...
  const unClaimedKswap = stakingInfo?.earnedAmount?.toFixed(6) ?? '0'
  ...
  const kswapRate = stakingInfo?.rewardRatePerWeek?.toSignificant(4, { groupSeparator: ',' })
  ...
  const yourKswapRate = {
    ...
    value: `${kswapRate}    ${t('earnPage.rewardPerWeek', { symbol: 'KSWAP' })}`
  }

  const unClaimedRow = {
    label: `${t('migratePage.unclaimedKswap')}`,
    value: userLiquidityUnstaked ? `${unClaimedKswap}` : '-'
  }
  const poolShareRow = {
  ...
  if (type === 'unstake') info = [yourKswapRate, unClaimedRow]
