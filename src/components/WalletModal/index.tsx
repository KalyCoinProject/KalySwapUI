      ...
      return (
        <UpperSection>
          ...
          <ContentWrapper>
            {web3Error instanceof UnsupportedChainIdError ? (
              <>
                <h5>{t('walletModal.pleaseConnectKalychain')}</h5>
                {isMetamaskOrCbWallet && (
                  <ButtonLight onClick={addKalychainNetwork}>{t('walletModal.switchKalychain')}</ButtonLight>
                )}
              </>
            ) : (
    ...
    return (
      <UpperSection>
        ...
        <ContentWrapper>
          ...
          {walletView !== WALLET_VIEWS.PENDING && (
            <Blurb>
              <span>{t('walletModal.newToKalychain')} &nbsp;</span>{' '}
